import { ChevronDown, GripVertical, Lock, FileText, PlusSquare, Receipt, Undo2, Layers } from 'lucide-react'
import { useCardCollapse } from '../../../hooks/useCardCollapse'

export function CollectionsMoreActionsCard() {
        ...
        >
          <div className="card-collapse-inner">
            <div className="mt-3 flex flex-col gap-2">
              {/* 1. Record Other Receipt */}
              <a
                href="#record-other-receipt"
              ...
              >
                <Receipt className="size-4 text-sky-700 shrink-0" />
                <span className="truncate">Record Other Receipt</span>
              </a>
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <span className="truncate">Apply Payment</span>
                </div>
                <Lock className="size-3.5 text-slate-400 shrink-0" />
              </a>

              {/* 3. Create Billing Statement */}
              <a
                href="#create-billing-statement"
                className="group flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2 text-xs font-medium text-slate-700 shadow-2xs transition hover:border-slate-300 hover:bg-slate-100"
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <FileText className="size-4 text-slate-400 shrink-0" />
                  <span className="truncate">Create Billing Statement</span>
                </div>
                <Lock className="size-3.5 text-slate-400 shrink-0" />
              </a>

              {/* 4. Issue Customer Refund */}
              <a
                href="#issue-customer-refund"
                className="group flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2 text-xs font-medium text-slate-700 shadow-2xs transition hover:border-slate-300 hover:bg-slate-100"
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <Undo2 className="size-4 text-slate-400 shrink-0" />
                  <span className="truncate">Issue Customer Refund</span>
                </div>
                <Lock className="size-3.5 text-slate-400 shrink-0" />
              </a>
            </div>
